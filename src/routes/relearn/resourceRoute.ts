...
resourceRoute.post("/", authMiddleware, async (req: MyAuthRequest, res) => {
  try {
    const sentResource = plainToClass(Resource, req.body);
    ...
    if (sentResource.id) {
      ...
      // If removing a rating
        ...
        sentResource.position = await resourceRepo.getLastPosition(
          sentResource.tag,
          user
        );
      }
...
resourceRoute.delete(
  ...
  async (req: MyAuthRequest, res) => {
    ...
    try {
      ...
      if (result.affected) {
        return res.status(200).json(`Resource ${resourceId} deleted.`);
      } else {
        return res
          .status(400)
          .json(
            new MyErrorsResponse("Resource id not found, or user is not owner.")
          );
      }
    } catch (err) {
      myConsoleError(err.message);
...
resourceRoute.post(
  ...
  async (req: MyAuthRequest, res) => {
    ...
    try {
      // ownership
      const verifiedResources = await resourceRepo.find({
        where: {
      ...
      if (verifiedResources.length !== sentResources.length) {
        return res
          .status(400)
          .json(new MyErrorsResponse("User does not own all sent resources."));
      }
    ...
    try {
      ...
      if (resource) {
        if (resource.completedAt.length > 0) {
          return res
            .status(400)
            .json(new MyErrorsResponse("Can't duplicate completed resources."));
        }

        // empurra todos após o :id
        await resourceRepo.increasePositionByOne(
          resource.tagId,
          user,
          resource.position + 1
        );
