...
skillRoute.post("/", authMiddleware, async (req: MyAuthRequest, res) => {
  ...
  try {
    ...
    await getRepository(SkillExpectation).delete({ skillId: savedSkill.id })
    await getRepository(SkillExpectation).save(expectations)
...
skillRoute.put("/:id", authMiddleware, async (req: MyAuthRequest, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json(new MyErrorsResponse("User is not owner or id doesn't exist"))
  } catch (err) {
    myConsoleError(err.message)
    return res.status(400).json(new MyErrorsResponse(err.message))
  }
