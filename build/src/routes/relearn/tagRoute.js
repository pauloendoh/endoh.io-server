...
tagRoute.post('/', authMiddleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // checking ownership
        if (sentTag.id) {
            const isOwner = yield tagRepo.find({ id: sentTag.id, user });
            if (!isOwner) {
            ...
            const nameExists = yield tagRepo.findOne({ name: sentTag.name, user: req.user });
            if (nameExists) {
        ...
        yield tagRepo.save(sentTag);
        const tags = yield tagRepo.getAllTagsFromUser(user);
...
tagRoute.get('/', authMiddleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const tagRepo = typeorm_1.getCustomRepository(TagRepository_1.default);
    const user = req.user;
    try {
        const tags = yield tagRepo.getAllTagsFromUser(user);
...
tagRoute.delete('/:id', authMiddleware_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const tagRepo = typeorm_1.getCustomRepository(TagRepository_1.default);
    const { user } = req;
    const tagId = parseFloat(req.params.id);
    try {
        const result = yield tagRepo.delete({ id: tagId, user });
        if (result.affected) {
